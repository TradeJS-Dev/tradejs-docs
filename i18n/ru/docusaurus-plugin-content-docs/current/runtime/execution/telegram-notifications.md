...
- Если `APP_URL` на HTTPS, TradeJS локально рендерит скриншот дашборда и загружает его в Telegram вместе с подписью.
...
- Приходит текст, но без картинки:
  проверьте `APP_URL`, генерацию скриншотов и возможность отрисовать страницу дашборда, по которой делается снимок.
